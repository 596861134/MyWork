...
import com.camera.opengles.camera.Renderer;
import com.camera.opengles.camera.TextureController;
import com.camera.opengles.filter.Beauty;
import com.camera.opengles.filter.LookupFilter;
import com.camera.opengles.interfaces.FrameCallback;
import com.camera.opengles.utils.PermissionUtils;
...
public class MainActivity extends AppCompatActivity implements FrameCallback {
    ...
    private Runnable initViewRunnable = new Runnable() {
        ...
        public void run() {
            ...
            mController = new TextureController(MainActivity.this);
            onFilterSet(mController);
            mController.setFrameCallback(720, 1280, MainActivity.this);
    ...
    public void saveBitmap(Bitmap b){
        ...
        if(!folder.exists()&&!folder.mkdirs()){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MainActivity.this, "无法保存照片", Toast.LENGTH_SHORT).show();
                }
            });
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(MainActivity.this, "保存成功->"+jpegName, Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private class Camera2Renderer implements Renderer {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            try {
                ...
                CameraCharacteristics c = mCameraManager.getCameraCharacteristics(1 + "");
